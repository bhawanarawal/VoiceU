...
const Elections: React.FC = () => {
 ...
 // Filter only active and upcoming elections
 ...
 const ElectionCard: React.FC<{ election: Election }> = ({ election }) => (
    <div className="bg-white/30 backdrop-blur-md rounded-2xl shadow-lg hover:shadow-2xl transform hover:-translate-y-2 transition-all duration-300 cursor-pointer overflow-hidden relative">
      ...
      <div className="bg-blue-600 p-4">
        <h3 className="text-white text-xl font-bold">{election.election_name}</h3>
      </div>

      {/* Body */}
      <div className="bg-white p-6 flex flex-col gap-4">
        <p className="text-gray-800">{election.description || "No description provided."}</p>
        ...
        <div className="text-gray-700 text-sm">
          <strong>Candidates Registered:</strong> {election.candidates?.length || 0}
        </div>
